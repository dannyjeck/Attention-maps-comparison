...
load('R_upper_fixtap'); % correlation of fixations resampled with the number of taps (expected upper limit on fix/tap correlation)
load('R_upper_intfix'); % correlation of interest resampled with the number of fixation
load('R_upper_inttap'); % correlation of interest resampled with the number of taps
load('R_upper_tapsal');
load('R_intNtaps_fix2');
load('R_upper_fixsal');
load('R_upper_fixruss');
load('R_upper_intsal');

% load('R_upper_fix35'); % R_upper_fixtap but with twice as many taps
% load('R_upper_int35'); R_upper_inttap = R_upper_int35; %R_upper_inttap but with twice as many taps
...
% subplot(2,4,1);
% plot(sort(Rfixint_rand(:)))
% hold on;
% plot(0.95*length(Rfixint_rand(:)),Rfixint,'r*');
% plot(0.95*length(Rfixint_rand(:)),Ravg(1,2),'g*');
% errorbar(0.95*length(Rfixint_rand(:)),Ravg(1,2),...
%     1.96*std(Rfixint)/sqrt(length(Rfixint)),'g');
% 
% plot(0.95*length(Rfixint_rand(:)),mean(R_upper_intfix(:)),'k*')
% hold off
% ylabel('Correlation value');
% xlabel('Position in sorted random distribution');
...
[pRfixtap_upper] = phistf(mean(R_upper_intfix,1),xlist,'FaceColor','k','FaceAlpha',alpha);
hold off;

plot(xlist,pRfixint_rand,'b')
hold all;
plot(xlist,pRfixint,'r')
plot(xlist,pRfixtap_upper,'k')
hold off
...
[~, p_Rfixint_upper] = ztest(mean(Rfixint),mean(R_upper_intfix(:)), ...
    sqrt(var(Rfixint(:))/length(Rfixint) + var(mean(R_upper_intfix))/length(mean(R_upper_intfix))),0.95,'both');
p_Rfixint_upper2 = (sum(mean(Rfixint)>=mean(R_upper_intfix,2))+1)/(size(R_upper_intfix,1)+1);

disp(['pfixint = ' num2str(p_Rfixint) ' / ' num2str(p_Rfixint_upper) '/' num2str(p_Rfixint_upper2)]);
...
herrorbar(mean(R_upper_intfix(:)),y,std(R_upper_intfix(:))/sqrt(length(Rfixint)),'ok');
ylim([0 y+.1])
hold off
...
% plot(0.95*length(Rfixtap_rand(:)),mean(R_upper_fixtap(:)),'k*');
...
pRfixtap_upper = phistf(mean(R_upper_fixtap,1),xlist,'FaceColor','k','FaceAlpha',alpha);
hold off;
...
[~, p_Rfixtap_upper] = ztest(mean(Rfixtap),mean(R_upper_fixtap(:)),...
    sqrt(var(Rfixtap(:))/length(Rfixtap) + var(mean(R_upper_fixtap))/length(mean(R_upper_fixtap))),0.95,'both');
p_Rfixtap_upper2 = (sum(mean(Rfixtap)>=mean(R_upper_fixtap,2))+1)/(size(R_upper_fixtap,1)+1);

title('Fixation vs Taps');
xlabel('Correlation Value');
ylabel('Relative Frequency')

disp(['pfixtap = ' num2str(p_Rfixtap) ' / ' num2str(p_Rfixtap_upper) '/' num2str(p_Rfixtap_upper2)]);
...
herrorbar(mean(R_upper_fixtap(:)),y,std(R_upper_fixtap(:))/sqrt(length(Rfixtap)),'ok');
ylim([0 y+.1])
...
% plot(0.95*length(Rinttap_rand(:)),mean(R_upper_inttap(:)),'k*');
% 
% hold off
...
pRinttap_upper = phistf(mean(R_upper_inttap,1),xlist,'FaceColor','k','FaceAlpha',alpha);
hold off;

plot(xlist,pRinttap_rand,'b')
hold all;
plot(xlist,pRinttap,'r')
plot(xlist,pRinttap_upper,'k')
hold off
...
[~, p_Rinttap_upper] = ztest(mean(Rinttap),mean(R_upper_inttap(:)),...
    sqrt(var(Rinttap(:))/length(Rinttap) + var(mean(R_upper_inttap))/length(mean(R_upper_inttap))),0.95,'both');
p_Rinttap_upper2 = (sum(mean(Rinttap)>=mean(R_upper_inttap,2))+1)/(size(R_upper_inttap,1)+1);
...
xlabel('Correlation Value');
% ylabel('Relative Frequency');
disp(['pinttap = ' num2str(p_Rinttap) ' / ' num2str(p_Rinttap_upper) ' / ' num2str(p_Rinttap_upper2)]);
...
herrorbar(mean(R_upper_inttap(:)),y,std(R_upper_inttap(:))/sqrt(length(Rinttap)),'ok');
ylim([0 y+.1])
...
% plot(0.95*length(Rtapsal_rand(:)),mean(R_upper_tapsal(:)),'k*');
% 
% hold off
...
pRtapsal_upper = phistf(mean(R_upper_tapsal,1),xlist,'FaceColor','k','FaceAlpha',alpha);
hold off;

plot(xlist,pRtapsal_rand,'b')
hold all;
plot(xlist,pRtapsal,'r')
plot(xlist,pRtapsal_upper,'k')
hold off
...
[~, p_Rtapsal_upper] = ztest(mean(Rtapsal),mean(R_upper_tapsal(:)),...
    sqrt(var(Rtapsal(:))/length(Rtapsal) + var(mean(R_upper_tapsal))/length(mean(R_upper_tapsal))),0.95,'both');
p_Rtapsal_upper2 = (sum(mean(Rtapsal)>=mean(R_upper_tapsal,2))+1)/(size(R_upper_tapsal,1)+1);

title('Comp Saliency vs Taps');
xlabel('Correlation Value');

disp(['ptapsal = ' num2str(p_Rtapsal) ' / ' num2str(p_Rtapsal_upper) ' / ' num2str(p_Rtapsal_upper2)]);
...
herrorbar(mean(R_upper_tapsal(:)),y,std(R_upper_tapsal(:))/sqrt(length(Rtapsal)),'ok');
ylim([0 y+.1])
...
[pRfixsal_upper] = phistf(mean(R_upper_fixsal,1),xlist,'FaceColor','k','FaceAlpha',alpha);
hold off;

plot(xlist,pRfixsal_rand,'b')
hold all;
plot(xlist,pRfixsal,'r')
plot(xlist,pRfixsal_upper,'k')
hold off
...
[~, p_Rfixsal_upper] = ztest(mean(Rfixsal),mean(R_upper_fixsal(:)),...
    sqrt(var(Rfixsal(:))/length(Rfixsal) + var(mean(R_upper_fixsal))/length(mean(R_upper_fixsal))),0.95,'both');
p_Rfixsal_upper2 = (sum(mean(Rfixsal)>=mean(R_upper_fixsal,2))+1)/(size(R_upper_fixsal,1)+1);


y=0.5;

disp(['pfixsal = ' num2str(p_Rfixsal) ' / ' num2str(p_Rfixsal_upper) ' / ' num2str(p_Rfixsal_upper2)]);
...
herrorbar(mean(R_upper_fixsal(:)),y,std(R_upper_fixsal(:))/sqrt(length(Rfixsal)),'ok');
ylim([0 y+.1])
...
[pRintsal_upper] = phistf(mean(R_upper_intsal,1),xlist,'FaceColor','k','FaceAlpha',alpha);
hold off;

plot(xlist,pRintsal_rand,'b')
hold all;
plot(xlist,pRintsal,'r')
plot(xlist,pRintsal_upper,'k')
hold off
...
[~, p_Rintsal_upper] = ztest(mean(Rintsal),mean(R_upper_intsal(:)),...
    sqrt(var(Rintsal(:))/length(Rintsal) + var(mean(R_upper_intsal))/length(mean(R_upper_intsal))),0.95,'both');
p_Rintsal_upper2 = (sum(mean(Rintsal)>=mean(R_upper_intsal,2))+1)/(size(R_upper_intsal,1)+1);

y=0.5;

disp(['pintsal = ' num2str(p_Rintsal) ' / ' num2str(p_Rintsal_upper) '/' num2str(p_Rintsal_upper2)]);
...
herrorbar(mean(R_upper_intsal(:)),y,std(R_upper_intsal(:))/sqrt(length(Rintsal)),'ok');
ylim([0 y+.1])
...
% pRfixruss_upper = phistf(mean(R_upper_fixruss,2),xlist,'FaceColor','k','FaceAlpha',alpha);
% hold off;
% 
% plot(xlist,pRfixruss_rand,'b')
% hold all;
% plot(xlist,pRfixruss,'r')
% plot(xlist,pRfixruss_upper,'k')
% hold off
% 
...
% [~, p_Rfixruss_upper] = ztest(mean(Rfixruss),mean(R_upper_fixruss(:)),std(mean(R_upper_fixruss,2)),0.95,'both');
% 
% 
% title('Russ Sal vs Fix');
% xlabel('Correlation Value');
% 
% disp(['pfixruss = ' num2str(p_Rfixruss) ' / ' num2str(p_Rfixruss_upper)]);
...
% herrorbar(mean(R_upper_fixruss(:)),y,std(R_upper_fixruss(:))/sqrt(length(Rfixruss)),'ok');
% 
% hold off
...
y=0.5;
hist(R_upper_fixtap(:,pic));

Pupper = phistf(R_upper_fixtap(:,pic),xlist);
Plower = phistf(Rfixtap_rand(:),xlist);

upper_err = std(R_upper_fixtap(:,pic));
...
herrorbar(mean(R_upper_fixtap(:,pic)),y,upper_err,'ok')
...
pupperbound = sum(R_upper_fixtap(:,pic)<Rfixtap(pic))/length(R_upper_fixtap(:,pic))
% title(num2str(pic));
